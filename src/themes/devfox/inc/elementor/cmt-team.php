...
class Devfox_Teambox_Widget extends Widget_Base{

	public function get_name() {
		return 'cmt_team_element';
	}
	
	public function get_title() {
		return esc_attr__( 'Team Box', 'devfox' );
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			'style',
			[
				'label'			=> esc_attr__( 'Select Teambox Style', 'devfox' ),
		...
		$this->add_control(
			...
			[
				...
				'default' => esc_attr__( 'Our Team', 'devfox' ),
		...
		if ( $posts->have_posts() ) {
			...
			<div class="cymolthemes-box-heading-wrapper <?php echo cymolthemes_wp_kses($headingclass); ?>">
				...
				if( function_exists('cymolthemes_get_filterbutton') ){
					$sortable_category_html = cymolthemes_get_filterbutton( $settings, 'cmt_team_group' );
	...
	protected function select_category() {
	  	$category = get_terms( array( 'taxonomy' => 'cmt_team_group', 'hide_empty' => false ) );
	  	$cat = array();
...
// Register widget
Plugin::instance()->widgets_manager->register_widget_type( new Devfox_Teambox_Widget() );
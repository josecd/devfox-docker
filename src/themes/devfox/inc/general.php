...
function cymolthemes_megamenu_override_default_theme($value) {
  ...
  if ( !isset($value['cymolthemes-main-menu']['theme']) ) {
    $value['cymolthemes-main-menu']['theme'] = 'devfox_theme_settings'; // change my_custom_theme_key to the ID of your exported theme
  }
...
function cymolthemes_mmmenu_breakpoint( $options ){
	...
	if( empty($megamenu_themes) ){
		$megamenu_themes = array (
			'devfox_theme_settings' => 
				array (
				'title' => esc_attr__('Devfox Theme Settings', 'devfox'),
				'arrow_up' => 'dash-f343',
...
function cymolthemes_image_sizes(){
	...
	add_image_size( 'cymolthemes-img-portfolio-style4', 646, 456, true);
	add_image_size( 'cymolthemes-img-size-two', 700, 466, true);
	add_image_size( 'cymolthemes-img-blog-stylethree', 570, 327, true);
	add_image_size( 'cymolthemes-img-blog-stylefour', 1140, 880, true);
}
}
...
function cymolthemes_single_skin_color(){
	...
	if( $post_id ){
		...
		if( !empty($page_meta['skincolor']) ){
			$devfox_theme_options['skincolor'] = esc_attr($page_meta['skincolor']);
		}
	}
}
}
				...
				if( substr($meta, 0, strlen($prefix) ) == $prefix ){
					...
					if( in_array($meta_name, $all_options) ){
						if( cymolthemes_is_json($value[0]) && !is_numeric($value[0]) ){
							...
							$original_val = ( isset($devfox_theme_options[$meta_name]) ) ? $devfox_theme_options[$meta_name] : array() ;
							$final_val    = array_merge( $original_val, $final_val );
						} else {
							// string
							$final_val = $value[0];
						}
						$devfox_theme_options[$meta_name] = $final_val;
					}
...
/**
 * Flush out the transients used in {@see devfox_categorized_blog()}.
 *
 * @since Devfox 1.0
 */
if( !function_exists('devfox_category_transient_flusher') ){
function devfox_category_transient_flusher() {
	delete_transient( 'devfox_categories' );
}
}
add_action( 'edit_category', 'devfox_category_transient_flusher' );
add_action( 'save_post',     'devfox_category_transient_flusher' );
...
function cymolthemes_add_inline_dynamic_css(){
	...
	// Minify CSS style
	if( !empty( $devfox_theme_options['minify'] ) && esc_attr($devfox_theme_options['minify'])==true ){
		$css = cymolthemes_minify_css( $css );
	}
	
	// devfox core style
	wp_add_inline_style( 'devfox-core-style', $css );
...
if ( ! function_exists( 'devfox_excerpt_more' ) && ! is_admin() ) :
/**
 ...
 */
function devfox_excerpt_more( $more ) {
	$link = sprintf( '<a href="%1$s" class="more-link">%2$s</a>',
		esc_url( get_permalink( get_the_ID() ) ),
		sprintf( esc_attr__( 'Continue reading %s', 'devfox' ), '<span class="screen-reader-text">' . get_the_title( get_the_ID() ) . '</span>' )
		);
	return ' &hellip; ' . $link;
}
add_filter( 'excerpt_more', 'devfox_excerpt_more' );
endif;
...
function cymolthemes_body_classes($bodyClass){
	...
	if( isset($devfox_theme_options['titlebar_view']) && 
		($devfox_theme_options['titlebar_view'] == 'default' || $devfox_theme_options['titlebar_view'] == 'allleft' || $devfox_theme_options['titlebar_view'] == 'allright' ) &&
		isset($devfox_theme_options['breadcrumb_on_bottom']) &&
		$devfox_theme_options['breadcrumb_on_bottom'] == true
	){
	...
	if( !empty($devfox_theme_options['headerstyle']) ){
		$bodyClass[] = 'headerstyle-'.esc_attr($devfox_theme_options['headerstyle']);
	}
	
	// check if dark background set for container.
	if( isset($devfox_theme_options['inner_background']['background-color']) && trim($devfox_theme_options['inner_background']['background-color'])!='' && cymolthemes_check_dark_color(esc_attr($devfox_theme_options['inner_background']['background-color'])) ){
		$bodyClass[] = 'cmt-sboxdark-layout';
	}
	
	// show/hide separator line between links in dropdown menu
	if( isset($devfox_theme_options['dropdown_menu_separator']) && trim($devfox_theme_options['dropdown_menu_separator'])=='0' ){
		$bodyClass[] = 'cmt-sboxdropmenu-hide-sepline';
	}
	

	// Sticky Fotoer ON/OFF
	if( isset($devfox_theme_options['stickyfooter']) && $devfox_theme_options['stickyfooter']==true ){
		$bodyClass[] = 'cymolthemes-sticky-footer';
	}
	
	// Single Portfolio
	if( is_singular('portfolio') ){
		$portfolioView        = esc_attr($devfox_theme_options['portfolio_viewstyle']); // Global view
	...
	if( isset($devfox_theme_options['footer_cta_box']) && $devfox_theme_options['footer_cta_box']==true ){
		$bodyClass[] = sanitize_html_class('cymolthemes-footer-cta-yes');
	}
	
	if( isset($devfox_theme_options['blog_readmore_shape'])){
		$bodyClass[] = 'cmt-globalbutton-'.$devfox_theme_options['blog_readmore_shape'];
	}
	
	// Boxed / Wide
	if( isset($devfox_theme_options['layout']) && trim($devfox_theme_options['layout'])!='' ){
		if( $devfox_theme_options['layout']=='boxed' || $devfox_theme_options['layout']=='framed' || $devfox_theme_options['layout']=='rounded' ){
			$bodyClass[] = 'cymolthemes-boxed';
		}
		if( $devfox_theme_options['layout']=='framed' || $devfox_theme_options['layout']=='rounded' ){
			$bodyClass[] = 'cymolthemes-boxed-'.sanitize_html_class($devfox_theme_options['layout']);
		}
		
		$bodyClass[] = sanitize_html_class( 'cymolthemes-'.trim($devfox_theme_options['layout']));
		if( $devfox_theme_options['layout']=='fullwide' ){
			if( isset($devfox_theme_options['full_wide_elements']['content']) && $devfox_theme_options['full_wide_elements']['content']=='1' ){
	...
	if( isset($devfox_theme_options['logoimg_retina']['url']) && $devfox_theme_options['logoimg_retina']['url']!=''){
	...
	$headerstyle_global = '';
	$headerstyle_page   = '';
	if( isset($devfox_theme_options['headerstyle']) && trim($devfox_theme_options['headerstyle'])!='' ){
		$headerstyle_global = sanitize_html_class($devfox_theme_options['headerstyle']);
	}
	if( is_page() ){
	...
	if( isset($devfox_theme_options['show_topbar']) && $devfox_theme_options['show_topbar']==true ){
		$bodyClass[] = sanitize_html_class('cymolthemes-topbar-yes');
	} else {
		$bodyClass[] = sanitize_html_class('cymolthemes-topbar-no');
	}
	
	// One Page website
	if( isset($devfox_theme_options['one_page_site']) && $devfox_theme_options['one_page_site']==true ){

...
function cymolthemes_header_style_change_triggers(){
	...
	var infostack_column_one		  = '<div class="header-icon"> <div class="icon"><i class="cmt_devfox flaticon-phone-call"></i></div></div><div class="header-content"><h3>Call Us</h3><h5>+123 795 9841</h5></div>';
	var infostack_column_two		  = '<div class="header-icon"> <div class="icon"><i class="cmt_devfox flaticon-email"></i></div></div><div class="header-content"><h3>Send Us Email</h3><h5>info@example.com</h5></div>';
	var infostack_column_three		  = '<div class="header-icon"> <div class="icon"><i class="cmt_devfox flaticon-placeholder"></i></div></div><div class="header-content"><h3>Locate In</h3><h5>24 Fifth st, Los Angeles, USA</h5></div>';
	...
	jQuery('input[name="devfox_theme_options[headerstyle]"]').change(function() {
		...
		if (this.value == 'one') { // one
			jQuery('select[name="devfox_theme_options[header_bg_color]"]').val(header_bg_color).change();
			jQuery('input[name="devfox_theme_options[titlebar_background][color]"]').iris('color', titlebar_background_color);  // 
			jQuery('select[name="devfox_theme_options[titlebar_text_color]"]').val(titlebar_text_color);
			jQuery('input[name="devfox_theme_options[titlebar_height]"').val(titlebar_height);  // Titlebar height
			jQuery('input[name="devfox_theme_options[show_topbar]"]').prop('checked', true);  // Show topbar
			jQuery('input[name="devfox_theme_options[header_search]"]').prop('checked', true);  // Show header search
			jQuery('input[name="devfox_theme_options[mainmenufont][color]"').iris('color', mainmenufont_color);  // 
			jQuery('input[name="devfox_theme_options[header_height]"]').val(header_height);  // Header Height
			jQuery('input[name="devfox_theme_options[logoimg][id]"]').val('');
			jQuery('input[name="devfox_theme_options[logoimg][thumb-url]"]').val( imgurl+logoimg );
			jQuery('input[name="devfox_theme_options[logoimg][full-url]').val( imgurl+logoimg );
			jQuery('input[name="devfox_theme_options[logo_max_height]"]').val(logo_max_height);  //
			jQuery('select[name="devfox_theme_options[mainmenu_active_link_color]"]').val(mainmenu_active_link_color).change(); // Select
			jQuery('input[name="devfox_theme_options[stickymainmenufontcolor]"]').iris('color', stickymainmenufontcolor_color);  // 
			jQuery('select[name="devfox_theme_options[sticky_header_bg_color]"]').val(sticky_header_bg_color).change();		
			jQuery('input[name="devfox_theme_options[dropmenu_background][color]"]').iris('color', dropmenu_background_color);  // Dropdown Menu color
			jQuery('input[name="devfox_theme_options[dropdownmenufont][color]"]').iris('color', dropdownmenufont_color);  // Dropdown Menu color
			jQuery('textarea[name="devfox_theme_options[header_text]"]').val('');  //
				
		} else if ( this.value == 'two' || this.value == 'overlay' ) {  // Overlay
			jQuery('select[name="devfox_theme_options[header_bg_color]"]').val('darkgrey').change();
			jQuery('input[name="devfox_theme_options[titlebar_background][color]"]').iris('color', titlebar_background_color);  // 
			jQuery('select[name="devfox_theme_options[titlebar_text_color]"]').val(titlebar_text_color);
			jQuery('input[name="devfox_theme_options[show_topbar]"]').prop('checked', true);  // Show topbar
			jQuery('input[name="devfox_theme_options[titlebar_height]"').val('400');  // Titlebar height
			jQuery('input[name="devfox_theme_options[header_search]"]').prop('checked', true);  // Show header search
			jQuery('input[name="devfox_theme_options[mainmenufont][color]"').iris('color', '#ffffff');   // 
			jQuery('input[name="devfox_theme_options[header_height]"]').val('82');  // Header Height
			jQuery('input[name="devfox_theme_options[logoimg][id]"]').val('');
			jQuery('input[name="devfox_theme_options[logoimg][thumb-url]"]').val( imgurl+'logo-white.png' );
			jQuery('input[name="devfox_theme_options[logoimg][full-url]').val( imgurl+'logo-white.png' );
			jQuery('input[name="devfox_theme_options[logo_max_height]"]').val(logo_max_height);  //
			jQuery('select[name="devfox_theme_options[mainmenu_active_link_color]"]').val(mainmenu_active_link_color).change(); // Select
			jQuery('input[name="devfox_theme_options[stickymainmenufontcolor]"]').iris('color', '#ffffff');// 
			jQuery('select[name="devfox_theme_options[sticky_header_bg_color]"]').val('darkgrey').change();
			jQuery('input[name="devfox_theme_options[dropmenu_background][color]"]').iris('color', dropmenu_background_color); // Dropdown Menu color
			jQuery('input[name="devfox_theme_options[dropdownmenufont][color]"]').iris('color', dropdownmenufont_color);  
			jQuery('textarea[name="devfox_theme_options[header_text]"]').val('');  //
			...
			jQuery('select[name="devfox_theme_options[titlebar_text_color]"]').val(titlebar_text_color);
			jQuery('input[name="devfox_theme_options[show_topbar]"]').prop('checked', true);  // Show topbar
			jQuery('input[name="devfox_theme_options[titlebar_height]"').val('300');  // Titlebar height
			jQuery('input[name="devfox_theme_options[header_search]"]').prop('checked', false);  // Show header search
			jQuery('input[name="devfox_theme_options[mainmenufont][color]"').iris('color', '#ffffff');// 
			jQuery('input[name="devfox_theme_options[header_height]"]').val('140');  // Header Height
			jQuery('input[name="devfox_theme_options[logoimg][id]"]').val('');
			jQuery('input[name="devfox_theme_options[logoimg][thumb-url]"]').val( imgurl+'logo.png' );
			jQuery('input[name="devfox_theme_options[logoimg][full-url]').val( imgurl+'logo.png' );
			jQuery('input[name="devfox_theme_options[logo_max_height]"]').val('50');   //  //
			jQuery('select[name="devfox_theme_options[mainmenu_active_link_color]"]').val(mainmenu_active_link_color).change(); // Select
			jQuery('input[name="devfox_theme_options[stickymainmenufontcolor]"]').iris('color', '#ffffff');// 
			jQuery('select[name="devfox_theme_options[sticky_header_bg_color]"]').val('darkgrey').change();
			jQuery('select[name="devfox_theme_options[infostack_phone_text]"]').val(infostack_phone_text).change();
			jQuery('textarea[name="devfox_theme_options[infostack_column_one]"]').val(infostack_column_one);
			jQuery('textarea[name="devfox_theme_options[infostack_column_two]"]').val(infostack_column_two);
			jQuery('textarea[name="devfox_theme_options[infostack_column_three]"]').val(infostack_column_three);
			jQuery('select[name="devfox_theme_options[header_menu_bg_color]"]').val('darkgrey').change();
			jQuery('select[name="devfox_theme_options[sticky_header_menu_bg_color]"]').val('darkgrey').change();
			
			jQuery('input[name="devfox_theme_options[dropmenu_background][color]"]').iris('color', dropmenu_background_color);  // Dropdown Menu color
			jQuery('input[name="devfox_theme_options[dropdownmenufont][color]"]').iris('color', dropdownmenufont_color);  // Dropdown Menu color
			
		} else if (this.value == 'six') {
			jQuery('select[name="devfox_theme_options[header_bg_color]"]').val('skincolor').change();			
			jQuery('input[name="devfox_theme_options[titlebar_background][color]"]').iris('color', titlebar_background_color);  // 
			jQuery('select[name="devfox_theme_options[titlebar_text_color]"]').val(titlebar_text_color);
			jQuery('input[name="devfox_theme_options[show_topbar]"]').prop('checked', true);  // Show topbar
			jQuery('input[name="devfox_theme_options[titlebar_height]"').val('283');  // Titlebar height
			jQuery('input[name="devfox_theme_options[header_search]"]').prop('checked', true);  // Show header 
			jQuery('input[name="devfox_theme_options[header_height]"]').val('66');  // Header Height
			jQuery('input[name="devfox_theme_options[logoimg][id]"]').val('');
			jQuery('input[name="devfox_theme_options[logoimg][thumb-url]"]').val( imgurl+'logo2.png' );
			jQuery('input[name="devfox_theme_options[logoimg][full-url]').val( imgurl+'logo2.png' );
			jQuery('input[name="devfox_theme_options[mainmenufont][color]"').iris('color', '#ffffff');   // 
			jQuery('input[name="devfox_theme_options[stickymainmenufontcolor]"]').iris('color', '#ffffff');// 
			jQuery('input[name="devfox_theme_options[logo_max_height]"]').val('89');   //  //
			jQuery('select[name="devfox_theme_options[mainmenu_active_link_color]"]').val('custom').change(); // Select
			jQuery('select[name="devfox_theme_options[sticky_header_bg_color]"]').val('skincolor').change();	
			jQuery('input[name="devfox_theme_options[dropmenu_background][color]"]').iris('color', dropmenu_background_color);  // Dropdown Menu color
			jQuery('input[name="devfox_theme_options[dropdownmenufont][color]"]').iris('color', dropdownmenufont_color);  // Dropdown Menu color
...
function cymolthemes_show_hide_titlebar_textcolor(){
	...
	var $this      = jQuery( 'select[name="devfox_theme_options[titlebar_text_color]"]' );
	var $page_this = jQuery( 'select[name="_cymolthemes_metabox_group[titlebar_text_color]"]' );
	
	if( $this.length > 0 ){
		if( jQuery($this).val()=='custom' ){
			jQuery( 'input[name="devfox_theme_options[titlebar_heading_font][color]"]' ).closest('.cmt-sboxtypography-font-color-w').show();
			jQuery( 'input[name="devfox_theme_options[titlebar_subheading_font][color]"]' ).closest('.cmt-sboxtypography-font-color-w').show();
			jQuery( 'input[name="devfox_theme_options[titlebar_breadcrumb_font][color]"]' ).closest('.cmt-sboxtypography-font-color-w').show();
		} else {
			jQuery( 'input[name="devfox_theme_options[titlebar_heading_font][color]"]' ).closest('.cmt-sboxtypography-font-color-w').hide();
			jQuery( 'input[name="devfox_theme_options[titlebar_subheading_font][color]"]' ).closest('.cmt-sboxtypography-font-color-w').hide();
			jQuery( 'input[name="devfox_theme_options[titlebar_breadcrumb_font][color]"]' ).closest('.cmt-sboxtypography-font-color-w').hide();
		}
	}
...
function cymolthemes_show_hide_color_picker_background(){
	...
	jQuery(items).each(function(n,val){
		...
		var $dropdown_element   = jQuery( 'select[name="devfox_theme_options['+dropdown_name+']"]' );
		
		// show/hide the color picker on load
		if( $dropdown_element.val()=='custom' ){
			jQuery( 'input[name="devfox_theme_options['+background_name+'][color]"]' ).closest('.cmt-sboxbackground-color-w').show();
		} else {
			jQuery( 'input[name="devfox_theme_options['+background_name+'][color]"]' ).closest('.cmt-sboxbackground-color-w').hide();
		}
...
jQuery(document).ready( function() {
	...
	jQuery( 'select[name="devfox_theme_options[titlebar_text_color]"]' ).change(function() {  // Theme Options
		cymolthemes_show_hide_titlebar_textcolor();
	});
	...
	jQuery('input[type=radio][name="devfox_theme_options[loaderimg]"]:checked').closest('label').addClass('cmt-sboxpre-loader-option-selected');
	jQuery('input[type=radio][name="devfox_theme_options[loaderimg]"]').change(function() {
		jQuery('input[type=radio][name="devfox_theme_options[loaderimg]"]').closest('label').removeClass('cmt-sboxpre-loader-option-selected');

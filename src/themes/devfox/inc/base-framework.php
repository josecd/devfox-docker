 ...
 */
if( ! function_exists( 'devfox_cs_framework_init' ) ) {
function devfox_cs_framework_init(){
	// Codestar framework config
	defined('CS_OPTION'          ) or define('CS_OPTION',           'devfox_theme_options' );
...
add_action( 'init', 'devfox_cs_framework_init', 1 );
add_action( 'admin_init', 'devfox_cs_framework_init', 1 );
			...
			if( isset($section['fields']) && is_array($section['fields']) ){
				foreach( $section['fields'] as $field ){
					if( isset($field['id']) && isset($field['default']) ){
						...
						$devfox_theme_options[$field_id] = $field_val;
					}
				}
...
function cymolthemes_get_font_option_values($elements){
	...
	if( is_array($elements) && count($elements)>0 ){
		...
		foreach( $elements as $element ){
			$val = ( isset($devfox_theme_options[$element]) ) ? $devfox_theme_options[$element] : array() ;

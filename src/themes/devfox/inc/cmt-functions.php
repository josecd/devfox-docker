...
function cymolthemes_shared_social_list(){
	...
	$sociallist = array(
		...
		'youtube'      => esc_attr__('YouTube', 'devfox'),
		'flickr'       => esc_attr__('Flickr', 'devfox'),
		'facebook'     => esc_attr__('Facebook', 'devfox'),
		'linkedin'     => esc_attr__('LinkedIn', 'devfox'),
		'gplus'        => esc_attr__('Google+', 'devfox'),
		'yelp'         => esc_attr__('Yelp', 'devfox'),
		'dribbble'     => esc_attr__('Dribbble', 'devfox'),
		'pinterest'    => esc_attr__('Pinterest', 'devfox'),
		'podcast'      => esc_attr__('Podcast', 'devfox'),
		'instagram'    => esc_attr__('Instagram', 'devfox'),
		'xing'         => esc_attr__('Xing', 'devfox'),
		'vimeo'        => esc_attr__('Vimeo', 'devfox'),
		'vk'           => esc_attr__('VK', 'devfox'),
		'houzz'        => esc_attr__('Houzz', 'devfox'),
		'issuu'        => esc_attr__('Issuu', 'devfox'),
		'google-drive' => esc_attr__('Google Drive', 'devfox'),
	);
...
function cymolthemes_a_color(){
	...
	a,.cmt-sbox .cmt-custom-heading a, .entry-header:not(.cmt-titlebar) .entry-title > a{color:<?php echo esc_attr($normal_color); ?>;}
	a:hover,.cmt-sbox .cmt-custom-heading a:hover, .entry-header:not(.cmt-titlebar) .entry-title > a:hover{color:<?php echo esc_attr($hover_color); ?>;}
...
function cymolthemes_pagination( $wp_query_data=false ){
	...
	$paginateLinks = paginate_links( array(
		...
		'type'      => 'array',
		'prev_text' => '<i class="cmt-devfox-icon-arrow-left"></i> <span class="cmt-hide cmt-sboxpagination-text cmt-sboxpagination-text-prev">' . esc_attr__( 'Previous page', 'devfox' ) . '</span>',
		'next_text' => '<span class="cmt-hide cmt-sboxpagination-text cmt-sboxpagination-text-next">' . esc_attr__( 'Next page', 'devfox' ) . '</span> <i class="cmt-devfox-icon-arrow-right"></i>',
	) );
...
function cymolthemes_get_option( $option, $inner_option='' ){
	...
	if( !is_array($devfox_theme_options) ){
		
		if( is_multisite() ){
			$devfox_theme_options = get_site_option('devfox_theme_options');
		} else {
	...
	if( isset($devfox_theme_options[$option]) ){
		$return = $devfox_theme_options[$option];
	} else {
...
function cymolthemes_get_all_registered_sidebars(){
	...
	$return = array( '' => esc_attr__('Default', 'devfox') );
...
/**
 * Custom template tags for Devfox
 *
...
if ( ! function_exists( 'devfox_comment_nav' ) ) :
/**
 * Display navigation to next/previous comments when applicable.
 *
 * @since Devfox 1.0
 */
function devfox_comment_nav() {
	// Are there comments to navigate through?
	if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) :
	?>
	<nav class="navigation comment-navigation">
		<h2 class="screen-reader-text"><?php esc_attr_e( 'Comment navigation', 'devfox' ); ?></h2>
		<div class="nav-links">
			<?php
				if ( $prev_link = get_previous_comments_link( esc_attr__( 'Older Comments', 'devfox' ) ) ) :
					printf( '<div class="nav-previous">%s</div>', $prev_link );
				endif;

				if ( $next_link = get_next_comments_link( esc_attr__( 'Newer Comments', 'devfox' ) ) ) :
...
if ( ! function_exists( 'devfox_entry_meta' ) ) :
/**
 * Prints HTML with meta information for the categories, tags.
 *
 * @since Devfox 1.0
 */
function devfox_entry_meta( $metafor="blogbox" ) {
			...
			switch( $meta_id ){
				...
				case 'date':
					...
					if ( in_array( get_post_type(), array( 'post', 'attachment' ) ) ) {
						...
						if( is_single() ) { $datelink = '%3$s'; } else { $datelink = '<a href="%2$s" rel="bookmark">%3$s</a>'; }
						$return .= sprintf( '<span class="cmt-meta-line posted-on"><i class="cmt-devfox-icon-calendar"></i> <span class="screen-reader-text cmt-hide">%1$s </span>' . $datelink . '</span>',
							esc_attr_x( 'Posted on', 'Used before publish date.', 'devfox' ),
				...
				case 'author':
					...
					if ( 'post' === get_post_type() ) {
						...
						$return .= sprintf( '<span class="cmt-meta-line byline"><i class="cmt-devfox-icon-user"></i> <span class="author vcard"><span class="screen-reader-text cmt-hide">%1$s </span>%2$s</span></span>',
							esc_attr_x( 'Author', 'Used before post author name.', 'devfox' ),
							$author
						);
					}
				...
				case 'comment':
					...
					if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
						$return .= '<span class="cmt-meta-line comments-link"><i class="cmt-devfox-icon-comment-empty"></i> ';
						ob_start();
						comments_popup_link( esc_attr__( '0 Comments', 'devfox' ) );
				...
				case 'cat':
					...
					if ( !empty($categories_list) ) {
						...
						$return .= sprintf( '<span class="cmt-meta-line cat-links"><i class="cmt-devfox-icon-folder-open-empty"></i> <span class="screen-reader-text cmt-hide">%1$s </span>%2$s</span>',
							esc_attr_x( 'Categories', 'Used before category names.', 'devfox' ),
							$categories_list
						);
					}
				...
				case 'tag':
					$tags_list = get_the_tag_list( '', esc_attr_x( ', ', 'Used between list items, there is a space after the comma.', 'devfox' ) );
					if ( !empty($tags_list) ) {
						if( $tag_link!=true ){
						...
						$return .= sprintf( '<span class="cmt-meta-line tags-links"><i class="fa fa-tag"></i> <span class="screen-reader-text cmt-hide">%1$s </span>%2$s</span>',
							esc_attr_x( 'Tags', 'Used before tag names.', 'devfox' ),
							$tags_list
						);
					}
 ...
 */
if( ! function_exists('devfox_categorized_blog') ){
function devfox_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'devfox_categories' ) ) ) {
		...
		set_transient( 'devfox_categories', $all_the_cool_cats );
	}

	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so devfox_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so devfox_categorized_blog should return false.
		return false;
	}
}
}
...
function cymolthemes_blogbox_comment_count( $echo = false ) {
	...
	if( comments_open() ){
		...
		$return  .= '<div class="cymolthemes-blogbox-comment"><i class="cmt-devfox-icon-comment"></i> '. $comments .'</div>';
	}
	return $return;
}
}
	...
	if( !empty($type) ){
		...
		if( !empty($return) ){
			...
			switch( $type ){
				...
				case 'phone':
					$return = '<div class="cmt-team-list-title"><i class="cmt-devfox-icon-phone"></i> '. esc_attr__('Phone','devfox') .' :</div>
						<div class="cmt-team-list-value"><a href="tel:' . str_replace(' ', '', $return) . '">' . $return . '</a></div>';
					break;
					
				case 'email':
					$return = '<div class="cmt-team-list-title"><i class="cmt-devfox-icon-mail"></i> '. esc_attr__('Email','devfox') .' :</div>
				...
				case 'website':
					...
					$return = '<div class="cmt-team-list-title"><i class="cmt-devfox-icon-world"></i> '. esc_attr__('Website','devfox') .' :</div>
...
function cymolthemes_create_icon_from_data( $data, $i_tag_only=false ){
	...
	if( isset($data['library']) && isset($data['library_fontawesome']) && isset($data['library_linecons']) && isset($data['library_themify']) ){
		...
		$library_cmt_devfox = ( !empty($data['library_cmt_devfox']) ) ? $data['library_cmt_devfox'] : '';
		...
		if( !empty(${'library_'.$library}) && ${'library_'.$library}!='empty' ){
			...
			$return = do_shortcode('[cmt-sboxicon type="'.$library.'" icon_cmt_devfox="'.$library_cmt_devfox.'" icon_fontawesome="'.$library_fontawesome.'" icon_linecons="'.$library_linecons.'" icon_themify="'.$library_themify.'"]');
...
function cymolthemes_portfolio_next_prev_btn() {
	...
	if( is_singular('cmt_portfolio') ){
		...
		$return = get_the_post_navigation( array(
			'next_text' => '<span class="meta-nav" aria-hidden="true">' . esc_attr__( 'Next', 'devfox' ) . '</span> ' .
				'<span class="screen-reader-text cmt-hide">' . esc_attr__( 'Next post:', 'devfox' ) . '</span> ' .
				'<span class="post-title cmt-hide">%title</span>',
			'prev_text' => '<span class="meta-nav" aria-hidden="true">' . esc_attr__( 'Previous', 'devfox' ) . '</span> ' .
				'<span class="screen-reader-text cmt-hide">' . esc_attr__( 'Previous post:', 'devfox' ) . '</span> ' .
				'<span class="post-title cmt-hide">%title</span>',
		) );
...
function cymolthemes_portfolio_detailsbox(){
	...
	if( isset($pf_details_line) && is_array($pf_details_line) && count($pf_details_line)>0 ){
		foreach( $pf_details_line as $key=>$val ){
			...
			if( !empty($val['pf_details_line_title']) ){ $row_title = sprintf( esc_attr__('%s ', 'devfox'), $val['pf_details_line_title'] ); }
			...
			if( !empty($row_value) ){
				$return .= '
						<li class="cmt-sboxpf-details-date">
							<span class="cmt-sboxpf-left-details">'. $row_title .'</span>
...
function cymolthemes_portfoliobox_media_link( $default_icon='fa fa-search', $video_icon='cmt-devfox-icon-video', $audio_icon='cmt-devfox-icon-music-alt', $slider_icon='cmt-devfox-icon-gallery-1' ){

	$icon_code='<i class="cmt-devfox-icon-plus-1"></i>';
...
function cymolthemes_gallerystyle_portfoliobox_media_link( $default_icon='cmt-devfox-icon-search', $video_icon='cmt-devfox-icon-video', $audio_icon='cmt-devfox-icon-music-alt', $slider_icon='cmt-devfox-icon-gallery-1' ){

	$icon_code='<i class="cmt-devfox-icon-search"></i>';
	if( !empty($default_icon) ){
		$icon_code='<i class="' . $default_icon . '"></i>';
	}
...
function cymolthemes_get_post_format_icon( $post_id='' ){
	...
	if( $post_type=='post' ){
		...
		$return = '<i class="cmt-devfox-icon-' . $icon_class . '"></i>';
...
function cymolthemes_get_boxes( $cpt='blog', $vars=array() ){
	...
	$allword  = (!empty($vars['allword'])) ? $vars['allword'] : esc_attr__('All', 'devfox');
...
function cymolthemes_global_team_member_template_list( $for_vc=false ){
	$return = array(	
				array(
					'label'	=> esc_attr__('Teambox Style 1','devfox'),
					'value'	=> 'styleone',
					'thumb'	=> get_template_directory_uri() . '/inc/images/teambox-style1.png',
				),
				array(
					'label'	=> esc_attr__('Teambox Style 2','devfox'),
					'value'	=> 'styletwo',
...
function cymolthemes_blogbox_readmore_text(){
	...
	$return             = esc_attr__('Read More', 'devfox');
...
function cymolthemes_servicebox_readmore_text(){
	...
	$return             = esc_attr__('More Details', 'devfox');
...
function cymolthemes_entry_icon() {
	...
	$iconCode .= '<i class="cmt-devfox-icon-'.$icon.'"></i>';
...
function cymolthemes_blogbox_single_meta( $option='date', $args=array() ) {
	...
	switch($option){
		...
		case 'category' :
			...
			$categories_list = get_the_category_list( esc_attr_x( ', ', 'Used between list items, there is a space after the comma.', 'devfox' ) );
			if ( !empty($categories_list) ) {
...
function cymolthemes_titlebar_title(){
	...
	if( is_singular() || is_home() ){  // single page, single post and single cpt
		...
		if( is_home() ){
			...
			$title = esc_attr__( 'Blog', 'devfox' );  // Setting for Titlebar title
		}
		...
		$adv_tbar_catarc = ( !empty($adv_tbar_catarc) ) ? esc_attr($adv_tbar_catarc.' %s') : esc_attr__('Category Archives: %s', 'devfox') ;
		$title = sprintf(
			$adv_tbar_catarc,
		...
		$adv_tbar_tagarc = ( !empty( $adv_tbar_tagarc ) ) ? esc_attr($adv_tbar_tagarc.' %s') : esc_attr__('Tag Archives: %s','devfox') ;
		$title = sprintf(
			$adv_tbar_tagarc,
		...
		if( is_tax('cmt_team_group') || is_tax('cmt_portfolio_category') ){
			...
			global $wp_query;
			$adv_tbar_postclassified = ( !empty($adv_tbar_postclassified) ) ? esc_attr($adv_tbar_postclassified.' %s') : esc_attr__('Posts classified under: %s', 'devfox') ;
		...
		$adv_tbar_authorarc = ( !empty( $adv_tbar_authorarc ) ) ? esc_attr($adv_tbar_authorarc.' %s') : esc_attr__('Author Archives: %s', 'devfox');
		...
		$title    = sprintf( esc_attr__( 'Search Results for %s', 'devfox' ), '<span class="cmt-titlebar-heading cmt-sboxtbar-search-title">' . get_search_query() . '</span>' );


	} else if( is_404() ){ // 404
		$error404_big_text = cymolthemes_get_option('error404_big_text');
		$title             = sprintf( esc_attr__( '%s ', 'devfox' ), $error404_big_text );
		...
		if( function_exists('tribe_is_month') && tribe_is_month() && !is_tax() ) { // The Main Calendar Page
			...
			global $wp_query;
			$title = esc_attr__( 'Events on: ', 'devfox' ). date('F j, Y', strtotime($wp_query->query_vars['eventDate']));
			...
			$title = sprintf( esc_attr__( 'Daily Archives: %s', 'devfox' ), '<span>' . get_the_date() . '</span>' );
		} elseif( is_month() ){
			$title = sprintf( esc_attr__( 'Monthly Archives: %s', 'devfox' ), '<span>' . get_the_date( esc_attr_x( 'F Y', 'monthly archives date format', 'devfox' ) ) . '</span>' );
		} elseif( is_year() ){
			$title = sprintf( esc_attr__( 'Yearly Archives: %s', 'devfox' ), '<span>' . get_the_date( esc_attr_x( 'Y', 'yearly archives date format', 'devfox' ) ) . '</span>' );
		} else {
				$title = esc_attr__( 'Archives', 'devfox' );			
		};
...
function cymolthemes_header_links(){
	...
	if( function_exists('is_woocommerce') && $wc_header_icon==true ){
		...
		$return  .= '<span class="cmt-header-icon cmt-header-wc-cart-link"><a href="' . $cart_url . '"><i class="cmt-devfox-icon-basket"></i><span class="number-cart">' . esc_attr($count) . '</span></a></span>';
	}
		
	// Search icon
	if( $header_search==true ){
		$class   = '';
		$return .= '<div class="cmt-header-icon cmt-sboxheader-search-link"><a href="#"><i class="cmt-devfox-icon-search-1"></i></a><div class="cmt-search-overlay"><form method="get" class="cmt-site-searchform" action=" ' . esc_url( home_url() ) . '"> <div class="w-search-form-h"><div class="w-search-form-row"><div class="w-search-input"><input type="search" class="field searchform-s" name="s" placeholder="' . esc_attr($search_input) . '" /><button type="submit"><span class="cmt-devfox-icon-search-1"></span></button></div></div></div></form><span class="cmt-close"><a href="#"><i class="cmt-devfox-icon-cancel-2"></i></a></span></div></div>';
	}
...
function cymolthemes_fbar_open_icon(){
	...
	$return = '<span class="cmt-sboxfbar-open-icon cmt-sboxicolor-' . cymolthemes_sanitize_html_classes( cymolthemes_get_option('fbar_icon_color') ) . '">' . cymolthemes_wp_kses($fbar_handler_icon)  .'</span>';
...
function cymolthemes_fbar_close_icon(){
	...
	$return = '<span class="cmt-devfox-icon-close cmt-sboxicolor-' . cymolthemes_sanitize_html_classes( cymolthemes_get_option('fbar_icon_color') ) . '" style="display:none;">' . cymolthemes_wp_kses($fbar_handler_icon_close)  .'</span>';
...
function cymolthemes_fbar_close_icon_for_content_area(){
	...
	$fbar_handler_icon_close = cymolthemes_get_option('fbar_handler_icon_close');
	$return = '<span class="cmt-devfox-icon-close cmt-sboxicolor-' . cymolthemes_sanitize_html_classes( cymolthemes_get_option('fbar_icon_color') ) . '">' . cymolthemes_create_icon_from_data( $fbar_handler_icon_close, true ) . '</span>';
...
function cymolthemes_footer_row_class( $row='first' ){
	...
	// BG color
	if( !empty($devfox_theme_options[$row.'_footer_bg_color']) ){
		$class .= ' cmt-bg cmt-bgcolor-'.sanitize_html_class($devfox_theme_options[$row.'_footer_bg_color']);
	}
	
	// Text color
	if( !empty($devfox_theme_options[$row.'_footer_text_color']) ){
		$class .= ' cmt-textcolor-'.sanitize_html_class($devfox_theme_options[$row.'_footer_text_color']);
	}
	
	// If bg image is there
	if( !empty($devfox_theme_options[$row.'_footer_bg_all']['image']) ){
	...
	// border type
	if( !empty($devfox_theme_options[$row.'_footer_bordertype']) ){
		$class .= ' cmt-sboxbordertype-'.sanitize_html_class($devfox_theme_options[$row.'_footer_bordertype']);
	}
	
	//widget border color
	if( !empty($devfox_theme_options[$row.'_footer_widget_bordercolor']) ){
		$class .= ' cmt-sboxwidgetbordercolor-'.sanitize_html_class($devfox_theme_options[$row.'_footer_widget_bordercolor']);
	}
...
function cymolthemes_box_team_social_links(){
	...
	if( !empty($data) && is_array($data) && count($data)>0 ){
		...
		foreach($data as $social ){
			...
			$return .= '<li><a href="'. $social['social_icons_list_link'] .'" target="_blank" data-tooltip="' . $social_name . '"><i class="cmt-devfox-icon-'. $social['social_icons_list_icon'] .'"></i><span class="cmt-hide">'. $social_name .'</span></a></li>';
		}
 ...
 */
if( !function_exists('devfox_addhttp') ){
	function devfox_addhttp($url){
		if (!preg_match("~^(?:f|ht)tps?://~i", $url)){
			$url = "http://" . $url;
		}
		return $url;
...
function cymolthemes_testimonial_featured_image($size='thumbnail'){
	...
	$return = ( !empty($featured_image) ) ? $featured_image : '<span class="cymolthemes-icon-box"><i class="demo-icon cmt-devfox-icon-quote-left"></i></span>';
...
function cymolthemes_star_ratting(){
	...
	for ($i = 1; $i <= 5; $i++) {
		if( $star_ratings >= $i ){
			$return .= ' <i class="cmt-devfox-icon-star-1 cmt-sboxactive"></i> ';
		} else {
			$return .= ' <i class="cmt-devfox-icon-star-empty-1"></i> ';
		}
	}
	return $return;
}
...
function cymolthemes_revslider_array( $countonly=false ) {
	...
	if( $countonly==true ){
		...
		if( count($sliders)==0 ){
			$sliders[''] = esc_attr__('(No Slider Found)', 'devfox');
		}
		return $sliders;
	}
}
	...
	if ( function_exists('lsSliders') ) {
		...
		if( $countonly == true ){
			...
			if( count($slider_names) == 0 ){
				$slider_names[''] = esc_attr__('(No Slider Found)', 'devfox');
			}
			return $slider_names;
		}
...
function cymolthemes_single_get_sidebar_value(){
	...
	$sidebar = $devfox_theme_options['sidebar_post'];
	...
	if( is_page() || is_singular() ){
		...
		if( !empty($devfox_theme_options['sidebar_'.$cpt]) ){
			$sidebar = $devfox_theme_options['sidebar_'.$cpt];
		}
	...
	if( is_search() ){
		$sidebar = $devfox_theme_options['sidebar_search'];
	}
...
function cymolthemes_event_description(){ 
	$return   = '';
	$readMore = esc_attr__('See Event', 'devfox') . '';
...
function cymolthemes_event_readmore(){ 
	$return   = '';
	$readMore = esc_attr__('View Details', 'devfox') . ' <i class="cmticon-fa-angle-right"></i>';
...
function cymolthemes_comment_row_template($comment, $args, $depth){
    ...
    <<?php echo esc_attr($tag); ?> <?php comment_class( empty( $args['has_children'] ) ? '' : 'parent' ) ?> id="comment-<?php comment_ID() ?>">
		...
		<?php if ( $comment->comment_approved == '0' ) : ?>
			 <em class="comment-awaiting-moderation"><?php esc_attr_e( 'Your comment is awaiting moderation.', 'devfox' ); ?></em>
			  <br />
		<?php endif; ?>
		<div class="comment-box">
			<div class="comment-meta commentmetadata">
				<?php printf( '<cite class="cmt-sboxcomment-owner fn">%s</cite> <span class="says">' . esc_attr__('says:', 'devfox' ) . '</span>', get_comment_author_link() ); ?>
				
				<a href="<?php echo htmlspecialchars( get_comment_link( $comment->comment_ID ) ); ?>">
				<?php
				printf( esc_attr__( '%1$s at %2$s', 'devfox' ), get_comment_date(),  get_comment_time() ); ?></a><?php edit_comment_link( esc_attr__( '(Edit)', 'devfox' ), '  ', '' );
				?>
			</div>			
...
function cymolthemes_author_social_links(){
	...
	foreach( $all_socials as $social_class => $social_link ){
		if( !empty($social_link) ){
			$return .= '<li><a href="'. $social_link .'" target="_blank"><i class="cmt-devfox-icon-'. $social_class .'"></i><span class="cmt-hide">'. ucwords($social_class) .'</span></a></li>';
		}
	}
...
function cymolthemes_search_results_box_title( $post_type='post' ){
	...
	$small_link = '<small><a href="'. esc_url(get_home_url()).'?s='.get_search_query().'" class="label label-default"><i class="cmt-devfox-icon-angle-left"></i> '.esc_attr__('Back to results','devfox').'</a></small>';
	if( empty($_GET['post_type']) ){
		$small_link = '<small><a href="'. esc_url(get_home_url()).'?s='.get_search_query().'&post_type=' . $post_type . '" class="label label-default">'.esc_attr__('View more','devfox').'</a></small>';
	}
...
function cymolthemes_footer_ctabox(){
	...
	if( !empty($devfox_theme_options['footer_cta_column_layout']) ){
...
function cymolthemes_wp_kses( $string, $allowed_html_type='' ){
	...
	$allowed_html = array(
		...
		'div' => array(
			...
			'data-cmt-loop'		=> array(),
			'data-cmt-autoplay'	=> array(),
			'data-cmt-centermode'=> array(),
			'data-cmt-nav'		=> array(),
			'data-cmt-dots'		=> array(),
			'data-cmt-slidestoscroll'=> array(),
			'data-cmt-autoplayspeed'=> array(),
			'data-cmt-margin'	=> array(),
		),
		'span'   => array(
			'class'  => array(),
			'style'  => array(),
...
function cymolthemes_entry_date( $echo = true ) {
	if ( has_post_format( array( 'chat', 'status' ) ) ){
		$format_prefix = esc_attr_x( '%1$s on %2$s', '1: post format name. 2: date', 'devfox' );
	} else {
		$format_prefix = '%2$s';
	}
...
if ( ! function_exists( 'devfox_posttime_meta' ) ) :
function devfox_posttime_meta( $metafor="blogbox" ) {
		...
		if( comments_open() && $show_comment == true ){
			$return .= sprintf( '
				<div class="cymolthemes-commentbox">
					<span class="comments"><i class="fa fa-comments-o"></i> %1$s '.esc_attr__( 'Comments', 'devfox' ).'</span>
				</div>',
				get_comments_number()
			);
		}	
...
function cymolthemes_get_socialbar_links( $tooltip_position='top' , $tooltip='yes' ){
	...
	$return = '';
	if( !empty($devfox_theme_options['social_icons_list']) ){
		foreach( $devfox_theme_options['social_icons_list'] as $socialicon ){
			...
			$return .= '<li class="cmt-social-' . $social_id . ' cmt-sboxsocialbox-i-wrapper"><a class="cmt-sboxsocialbox-icon-link cmt-sboxsocialbox-icon-link-' . $social_id . ' ' . sanitize_html_class($class) . '" target="_blank" href="' . $href . '"><span class="frame-hover"></span><i class="cmt-devfox-icon-' . $social_id . '"></i><span class="social_name">'.$social_name.'</span></a></li>' . "\n";
		}
	}
	...
	foreach( $socialArray as $key=>$value ){
		...
		if( $key == 'rss' ){
			if( isset($devfox_theme_options['rss']) && $devfox_theme_options['rss']=='1' ){
			...
			if( isset($devfox_theme_options[$key]) && trim($devfox_theme_options[$key])!='' ){
				$return .= '<li class="'.$key.' "><a target="_blank" href="'.esc_url($devfox_theme_options[$key]).'" data-tooltip="'.$value[1].'"><i class="cmt-social-icon-'.$value[0].'"></i></a></li>';
			}
		}
	}
...
function cymolthemes_infostack_header_content(){
	...
	$four_content  = cymolthemes_get_option('infostack_column_four');
	...
	$return .= ( !empty( $four_content ) ) ? '<div class="header-widget"><div class="header-widget-main">'.do_shortcode($four_content).'</div></div>' : '' ;
...
/*
 *  cymolthemes elementor support
 */
 
 
 if( !function_exists('cmt_ordinal') ){
function cmt_ordinal($number) {
...
if( !function_exists('cmt_element_container') ){
function cmt_element_container( $settings = array( 'position' => 'start', 'cpt' => 'blog', 'data' => array() ) ){

	$return 	 = '';
	$inner_class_array = array('cymolthemes-boxes-inner');
	...
	// Carousel
	$cmt_loop			= ( !empty($settings['data']['cmt-loop']) && $settings['data']['cmt-loop']=='1' ) ? '1' : '1' ;
	$cmt_autoplay		= ( !empty($settings['data']['cmt-autoplay']) && $settings['data']['cmt-autoplay']=='1' ) ? '1' : '0' ;
	$cmt_centermode			= ( !empty($settings['data']['cmt-centermode']) && $settings['data']['cmt-centermode']=='1' ) ? '1' : '0' ;
	...
	$cmt_autoplayspeed	= ( !empty($settings['data']['cmt-autoplayspeed']) ) ? trim($settings['data']['cmt-autoplayspeed']) : '4500' ;
	...
	if( $position=='start' ){
		...
		// Data tags
		$data_array = array();
		$data_array[] = 'data-cmt-show="'.$show.'"';
		$data_array[] = 'data-cmt-loop="'.$cmt_loop.'"';
		$data_array[] = 'data-cmt-autoplay="'.$cmt_autoplay.'"';
		$data_array[] = 'data-cmt-centermode="'.$cmt_centermode.'"';
		$data_array[] = 'data-cmt-nav="'.$carousel_nav.'"';
		$data_array[] = 'data-cmt-dots="'.$carousel_dots.'"';
		$data_array[] = 'data-cmt-slidestoscroll="'.$carousel_slidestoscroll.'"';
		$data_array[] = 'data-cmt-autoplayspeed="'. esc_attr($cmt_autoplayspeed).'"';
		$data_array[] = 'data-cmt-margin="'. esc_attr($gap).'"';
		$class_array[] = 'cymolthemes-boxes';
		$class_array[] = 'cymolthemes-boxes-'.$cpt;
		$class_array[] = 'cymolthemes-element-'.$cpt.'box-'.$style;
		$class_array[] = 'cymolthemes-boxes-view-'.$view_type;
		$class_array[] = 'cymolthemes-boxes-col-'. $column;
		...
		if( !empty($settings['data']['text_align']) ){
			$class_array[] = 'cymolthemes-boxes-textalign-'.$settings['data']['text_align'];
		}
		if( !empty($gap) ){
			$class_array[] = 'cymolthemes-boxes-spacing-'.$gap;
		}
		if( !empty($settings['data']['sortable']) ){
			$class_array[] = 'cymolthemes-sortable-' . esc_attr($settings['data']['sortable']);
		}
...
function cymolthemes_get_filterbutton( $atts=array(), $taxonomy='' ){
	...
	if( !empty($atts['sortable']) && $atts['sortable']=='yes' ){
		...
		$sortable_category_html .= '<li class="cmt-sortable-link cmt-sortable-all-link"><a class="selected" href="javascript:void(0);" data-filter="*"> ' . $atts['allword'] . ' </a></li>';
		...
		foreach( $post_terms as $term ){		
			$sortable_category_html .= '<li class="cmt-sortable-link"><a href="javascript:void(0);" data-filter=".' . esc_attr($term->slug) . '">' . esc_html($term->name) . '</a></li>';
		}

		$return = '<div class="cmt-sortable-wrapper cmt-sortable-wrapper-' . $taxonomy . '"><nav class="cmt-sortable-list"><nav class="cmt-sortable-list"><ul>'.$sortable_category_html.'</ul></nav></div>';
		return $return;
	}
}
}
...
if( !function_exists('cmt_element_block_container') ){
function cmt_element_block_container( $settings = array( 'position' => 'start', 'column' => 'three', 'cpt' => 'blog', ) ){
	$return = '';
 ...
 *
 */
if(!function_exists('cymolthemes_serviceicon_box')){
function cymolthemes_serviceicon_box(){
		$return      		= '';
	$cmticon_meta   = get_post_meta( get_the_id(), 'cymolthemes_tmservice_icon', true );
	$serviceicon 		= ( !empty($cmticon_meta['cmt_serviceicon']) ) ? $cmticon_meta['cmt_serviceicon'] : '' ;
		
	$return .= ( !empty($serviceicon) ) ? '<div class="cmt-service-iconbox"><span class="cmt-service-icon">' . $serviceicon . '</span></div>' : '';
 ...
 */
if( !function_exists('cymolthemes_404_iconcode') ){
function cymolthemes_404_iconcode(){
	$cmt_iconde = cymolthemes_get_option('error404_big_iconcode');
	$return  = ( !empty($cmt_iconde) ) ? '<div class="cmt-big-icon">' . cymolthemes_wp_kses( $cmt_iconde ) . '</div>' : '' ;
	return $return;
}
}
if( !function_exists('cymolthemes_404_imgdescription') ){
function cymolthemes_404_imgdescription(){
$error404_iamgebox_text = cymolthemes_get_option('error404_iamgebox_text');
$imgbox_text = cymolthemes_wp_kses( $error404_iamgebox_text );
$return = ( !empty($imgbox_text) ) ? '<div class="page-content"> <p>'.do_shortcode( $imgbox_text ).'</p> </div><!-- .page-content -->' : '' ;
return $return;
}
}
/**
 * Portfolio icon Detials
 *
 * Create your own cymolthemes_portfolioicon_box() to override in a child theme.
 *
 * @since Devfox 1.0
 *
 */
if(!function_exists('cymolthemes_portfolioicon_box')){
function cymolthemes_portfolioicon_box(){
	$return      		= '';
	$cmtpicon_meta   = get_post_meta( get_the_id(), 'cymolthemes_tmportfolio_icon', true );
	$portfolioicon 		= ( !empty($cmtpicon_meta['cmt_portfolioicon']) ) ? $cmtpicon_meta['cmt_portfolioicon'] : '' ;
		
	$return .= ( !empty($portfolioicon) ) ? '<div class="cmt-portfolio-iconbox"><span class="cmt-portfolio-icon">' . $portfolioicon . '</span></div>' : '';
 ...
 *
 */
if(!function_exists('cymolthemes_portfolioicon_boxs')){
function cymolthemes_portfolioicon_boxs(){
	$return      		= '';
	$cmtpicon_meta   = get_post_meta( get_the_id(), 'cymolthemes_cmtportfolio_icon', true );
	$portfolioicon 		= ( !empty($cmtpicon_meta['cmt_portfolioicon']) ) ? $cmtpicon_meta['cmt_portfolioicon'] : '' ;
